...
Pid: /data/mysql/var/run/mysql.pid
套接文件：/data/mysql/var/run/mysql.sock
...
四、日志文件
错误日志：/data/logs/mysql/mysql-error.log
慢日志：/data/logs/mysql/mysql-slow.log
...
状态查询 ：systemctl status mysqld

控制文件目录：/etc/systemd/system/mysqld.service
...
注：执行完上面SQL语句后，再执行：FLUSH PRIVILEGES; 否则不生效
